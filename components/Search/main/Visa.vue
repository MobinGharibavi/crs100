<template>
  ...
  >
    ...
    <div class="grid gap-4 grid-cols-4 grid-rows-2">
      <div
        ...
        :key="index"
        class="bg-white h-max hover:shadow-2xl group rounded-lg hover:rounded-t-lg hover:rounded-none gap-3 text-black relative p-2 flex flex-col"
      >
  ...
  <!-- mobile -->
  <section
    class="w-full lg:hidden flex px-5 py-2.5 gap-4 flex-col bg-orange text-white"
  >
    <div class="flex items-center rtl:flex-row-reverse justify-between">
      <h2 class="font-bold">{{ $t("search.main.visa.title") }}</h2>
      <div class="items-center rtl:flex-row-reverse flex gap-3">
        ...
        <button @click="() => items?.length - 1 > currentSlide && currentSlide++">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="0.66rem"
            height="1.5rem"
    ...
    >
      <Slide v-for="(item, index) in items" :key="index">
        <div
          class="bg-white h-max hover:shadow-2xl group rounded-lg gap-3 text-black p-2 flex flex-col w-[97%]"
        >
          <div class="rounded-lg overflow-hidden">
            <img :src="item.img" class="h-52 w-full" alt="..." />
          </div>
          ...
          <div class="flex rtl:flex-row-reverse justify-between items-center">
            <div class="flex gap-2 rtl:flex-row-reverse text-blue-500 items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="1.5rem"
                height="1.5rem"
